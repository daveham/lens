...
const debug = getDebugLog('manager.test');
...
describe('db manager', () => {
  ...
  test('add simulation', () => {
    ...
    return mgr.addSimulation(s).then(sim => {
  ...
  test('get simulation', () =>
    mgr.getSimulation(s1.id).then(sim => {
  ...
  test('get simulations for source', () =>
    mgr.getSimulationsForSource(sourceId).then(rows => {
      debug('simulations', rows);
      expect(rows).toHaveLength(1);
      expect(rows[0].id).toEqual(s1.id);
    }));
  ...
  test('add execution', () => {
    ...
    return mgr.addExecution(e).then(exe => {
      debug('execution', exe);
  ...
  test('get execution', () =>
    mgr.getExecution(e1.id).then(exe => {
      debug('execution', exe);
      expect(exe).toBeDefined();
  ...
  test('get executions', () =>
    mgr.getExecutions(s1.id).then(rows => {
  ...
  test('add rendering', () => {
    ...
    return mgr.addRendering(r).then(ren => {
      debug('rendering', ren);
  ...
  test('get rendering', () =>
    mgr.getRendering(r1.id).then(ren => {
      debug('rendering', ren);
      expect(ren).toBeDefined();
  ...
  test('get renderings', () =>
    mgr.getRenderings(e1.id).then(rows => {
